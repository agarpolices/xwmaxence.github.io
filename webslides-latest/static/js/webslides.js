...
window.addEventListener("DOMContentLoaded",e=>{
  let asdd = 0;
  const allElements = document.querySelectorAll('.sectionxd,.multiline,#reasonss,#title,#tennhom,#namebaihoc,#tacgia-container,.center-container,.lis,.block,#last');
...
if (elementToReveal) {
    ...
    if(clickCount >=3){
      ...
      if (elementToReveal !== document.querySelector("#a") && elementToReveal !== document.querySelector("#last"))
{    elementToReveal.appendChild(img.cloneNode(true));}
      console.log(asdd)
          if(asdd > 4){
            document.getElementById("col").style.display = 'none'
            if (asdd > 7)document.getElementById("col1").style.display = 'none'
...
document.addEventListener("click",  e => {
  ...
  case 3:
    if(asd === 3){
      ...
      const Elementsv1 = document.querySelectorAll('.block,.lis,#a,#last');
      urmom1(Elementsv1)
    break;
    }
}});
